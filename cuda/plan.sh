...
EOF
  ...
  ln -s "${pkg_prefix}/doc/pdf/EULA.pdf" "${pkg_prefix}/share/licenses/EULA.pdf"

  # Remove redundant man and samples
  rm -fr "${pkg_prefix}/doc/man"
  ...
  # Remove unused files
  rm -fr "${pkg_prefix}/"{bin,samples}"/.uninstall_manifest_do_not_delete.txt"
  rm -fr "${pkg_prefix}/samples/uninstall_cuda_samples"*.pl
  ...
  CUDA_RUN_PATH="$(pkg_path_for gcc-libs)/lib:$(pkg_path_for glibc)/lib"
  ...
  for bin in "${_cuda_bins[@]}"; do
    ...
    patchelf --interpreter "$(pkg_path_for glibc)/lib/ld-linux-x86-64.so.2" \
      --set-rpath "${pkg_prefix}/lib64:${pkg_prefix}/nvvm/lib64:${CUDA_RUN_PATH}" \
      "${pkg_prefix}/${bin}"
  ...
  for lib in "${_cuda_libs[@]}"; do
    ...
    patchelf --set-rpath "${CUDA_RUN_PATH}" \
      "${pkg_prefix}/lib64/${lib}.so"
  done

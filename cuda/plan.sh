...
do_install() {
  ...
  # Needs gcc7
  ln -s "$(pkg_path_for bdangit/gcc7)/bin/gcc" "${pkg_prefix}/bin/gcc"
  ln -s "$(pkg_path_for bdangit/gcc7)/bin/g++" "${pkg_prefix}/bin/g++"
...
EOF
  ...
  CUDA_RUN_PATH="${pkg_prefix}/lib64:${pkg_prefix}/nvvm/lib64:$(pkg_path_for gcc7-libs)/lib:$(pkg_path_for glibc)/lib"
